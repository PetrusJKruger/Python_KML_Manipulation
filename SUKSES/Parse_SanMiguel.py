...
for i in range(0,fldNum):
	fldName.append('SMG - ' + root.Document.Folder.Folder[i].name.text)
	print fldName[i]
	plcmNum = len(root.Document.Folder.Folder[i].Folder.Placemark)
	print plcmNum
	del myName
	myName = []
	for j in range(0,plcmNum):
		tmp1 = root.Document.Folder.Folder[i].Folder.Placemark[j].name.text
		tmp2 = unicode(tmp1[:1], "utf-8")
		if not tmp2.isnumeric():
			myName.append(fldName[i] + ' - ' + root.Document.Folder.Folder[i].Folder.Placemark[j].name.text)
			#print 'Blok  --> ' + myName[j]
			pm2 = KML.Placemark(
				KML.name(myName[j]),
				KML.Point(root.Document.Folder.Folder[i].Folder.Placemark[j].Point.coordinates
					)
				)
			...
			pm1 = KML.Placemark(
				...
				KML.Polygon(
					KML.tessellate(1),
					KML.outerBoundaryIs(
						KML.LinearRing(root.Document.Folder.Folder[i].Folder.Placemark[j].Polygon.outerBoundaryIs.LinearRing.coordinates)
						)
					)
				)
